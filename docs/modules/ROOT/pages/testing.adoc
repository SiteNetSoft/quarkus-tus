= Testing
:description: Testing an application that uses quarkus-tus: the in-memory store, REST-assured against the TUS endpoints, SSE, and integration tests in JVM and native mode.
...
TIP: A custom store needs no protocol knowledge — the extension fires `TusUploadCompletedEvent` and the other lifecycle events itself. To check that your store honours the SPI contract (staged writes, offsets, locking, concatenation), extend `AbstractUploadStoreContractTest` from the `quarkus-tus-tck` artifact; see xref:storage-spi.adoc#_verifying_your_store_the_contract_test[Custom Storage Backends].
