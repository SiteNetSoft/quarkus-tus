...
| `String`
| `creation,termination,checksum,checksum-trailer,expiration,concatenation,concatenation-unfinished,creation-with-upload,creation-defer-length`
...
body, instead of as a header. This matters for clients that stream a large chunk and cannot compute
its hash before sending it.

WARNING: Reading HTTP request trailers requires a `vertx-core` that implements
https://github.com/eclipse-vertx/vert.x/issues/5253[eclipse-vertx/vert.x#5253], which Vert.x does not
yet ship. Against a stock Vert.x the trailer is never seen and the chunk would be accepted unverified,
so remove `checksum-trailer` from `quarkus.tus.extensions` unless you are running a patched build.
