...
Opens an SSE connection for the given upload. The server sends a `connected` event immediately upon connection, followed by progress events as chunks are received.
...
Returns the current upload progress as a JSON object (non-streaming, single response). Useful for polling-based clients.
