...
Create a CDI bean annotated with `@Alternative` and `@Priority` to override the default `LocalFileUploadStore`:
...
writeChunkAsync:: Returns a `Uni<Long>` with the new offset after writing. Return `-1` if the upload was not found. Throw `LocalFileUploadStore.ChecksumMismatchException` if checksum validation fails (the extension handles the 460 response).
...
* Each upload gets a file named `{uploadId}.bin` in the configured directory
