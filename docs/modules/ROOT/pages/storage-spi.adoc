...
* The `Uni`/`Multi` methods are subscribed to on a Vert.x event loop: do not block in them.
...
public class S3UploadStore implements UploadStore {
    ...
    public Uni<Long> stageChunk(String id, long offset, Multi<Buffer> data, long expectedLength) {
        UploadInfo info = findUploadInfo(id).orElseThrow(() -> new UploadNotFoundException(id));
        if (offset != info.getOffset()) {
            throw new OffsetMismatchException("stale offset", info.getOffset());
        }
